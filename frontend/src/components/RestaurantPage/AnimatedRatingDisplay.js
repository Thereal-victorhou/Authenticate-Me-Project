...
export default function AnimatedRatingDisplay({ reviews }) {
  ...
  let maxVal = -Infinity;
	...
	const calculatePercentage = (reviews) => {
		if (reviews) {
			reviews.forEach((review) => {           // find quanity of each rating
				ratingObj[review.rating] += 1;
			});

			for (let rating in ratingObj) {       // Change values in ratingsObj to percentage
	...
	useEffect(() => {
      ...
      refArr.forEach(ref => {
        ...
        const durationForBar = (2 * maxVal) / val;
        // console.log('duration == ', durationForBar)
        ref.current.style.opacity = val === 0 ? 0 : '1';
        ref.current.style.transitionDuration = `${durationForBar === Infinity ? 0 : durationForBar}s`;
